...
class Tank extends Entity {
    ...
    drive(direction) {
        if (this.owningPlayer.stats.gas < 1)
            return;
...
class Player {
    constructor(room, name, color, ground) {
        ...
        this.stats = { weapons: [
                new Weapon(99, SmallMissile, "Small missle", 100, document.getElementById('')),
                new Weapon(10, VolcanoBomb, "Volcano bomb", 1000, document.getElementById('')),
                new Weapon(10, SmallAtomBomb, "Small atom bomb", 9000, document.getElementById('')),
                new Weapon(10, AtomBomb, "Atom bomb", 20000, document.getElementById(''))
            ],
            weaponIndex: 1,
            gas: 100,
            health: 100,
    ...
    nextWeapon(direction) {
        ...
        if (this.stats.weaponIndex < 0)
            this.stats.weaponIndex = this.stats.weapons.length - 1;
...
class GameRoom extends Room {
    ...
    onEnter(passed, backgroundContext) {
        ...
        this.angleElement = new SliderElement(this, this.width / 3 + 150, 30, 1 - (this.player.stats.angle / Math.PI), { width: "150px", height: "10px", backgroundImage: "url(../img/gradient-horizontal.png)", backgroundSize: "cover", backgroundRepeat: "no-repeat" }, (event) => {
        ...
        new DivElement(this, this.width / 2 + 260, 40, { textAlign: "left", width: "220px" }, (element) => {
            var _a;
            element.innerText = (_a = this.player.stats.weapons[this.player.stats.weaponIndex]) === null || _a === void 0 ? void 0 : _a.name;
        });
        // weapon count
        new DivElement(this, this.width / 2 + 383, 40, { textAlign: "left" }, (element) => {
            var _a;
            element.innerText = (_a = this.player.stats.weapons[this.player.stats.weaponIndex]) === null || _a === void 0 ? void 0 : _a.amount;
        });
