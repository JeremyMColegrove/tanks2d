...
class Ground {
    constructor(game, context, color) {
        this.game = game
        this.color = color
    ...
    getHarry(scale, magnitude, offset, levels) {
        var harry = new Array(this.game.width).fill(offset)
    ...
    generateGround(rgb, img) {
        ...
        for (var w=0; w<this.game.width; w++) {
            for (var h=0; h<this.game.height; h++) {
                if (h > this.game.height - this.harry[w]) {
                    const red = h * (this.game.width * 4) + w * 4;
                    array[red    ] = rgb[0] // red
        ...
        for (var w=x-radius; w<x+radius; w++) {
            ...
            for (var h=0; h<this.game.height; h++) {
                ...
                if (!hitGround && this.img.data[red + 3] > 0) {
                    hitGround = true
                    this.harry[w] = this.game.height - h + 1
    ...
    smoothHarry (x, radius, distance) {
        ...
        for (var point = x - smoothing; point < x + smoothing && point > 0 && point < this.game.width; point ++) {
            var average = 0
            var weight = 1 - (Math.abs(x - point)/(smoothing))
            var count = 0
            for (var sample = point-distance; sample < point+distance && sample > 0 && sample < this.game.width; sample++) {
